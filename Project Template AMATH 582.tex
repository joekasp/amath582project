...
\begin{figure}[h!] \label{fig:isamp}
	\centering
	\includegraphics[width = 7cm]{Idealsamp.png}
	\caption{A ``full" idealized signal and an example of a short sample taken.}
\end{figure}
Having \emph{a priori} knowledge of the exact ``signal" allows us to determine the sampling rates and discrete transforms that result in the best approximations under ideal conditions. In Figure~\ref{fig:isamp} the signal is given by
...
\begin{figure}[h!] \label{fig:icomp}
	\centering
	\includegraphics[width = 9.5cm]{Idealcomp.png}
	\caption{Comparison of using the Fourier transform (dashed line) and applying compressed sensing (solid line) to sampled data.}
\end{figure}

A variety of sample lengths were tested using both the Fourier transform and compressed sensing algorithms. 
